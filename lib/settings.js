import { DEFAULT_AGENT_PERMISSIONS, DEFAULT_CONTEXT_OPTIONS } from './types';
...
const LEGACY_DEFAULT_USER_INSTRUCTION = 'If a new feature is requested, add an explanation of how to use it into the summary and as comment in the code. Comment all code you generate or modify following best programming practices.';
...
function normalizeAgentPermissions(value) {
    ...
    return {
        ...
        policyBoundariesEnabled: parsed.policyBoundariesEnabled === undefined
            ? DEFAULT_AGENT_PERMISSIONS.policyBoundariesEnabled
            : Boolean(parsed.policyBoundariesEnabled),
        patternMode: parsed.patternMode === 'blacklist' ? 'blacklist' : 'whitelist',
        patterns: typeof parsed.patterns === 'string'
            ? parsed.patterns
            : DEFAULT_AGENT_PERMISSIONS.patterns
    };
}
function normalizeUndoHistoryLimit(value) {
...
export function loadSettings() {
    try {
        ...
        const storedReasoningEffort = parsed
        ...
        return {
            ...
            assistantShortcut: typeof parsed.assistantShortcut === 'string'
                ? parsed.assistantShortcut
                : DEFAULT_SHORTCUT,
            selectAboveShortcut: typeof parsed.selectAboveShortcut === 'string' &&
                parsed.selectAboveShortcut !== 'Ctrl+Shift+A'
                ? parsed.selectAboveShortcut
                : DEFAULT_SELECT_ABOVE_SHORTCUT,
            codexReasoningEffort: storedReasoningEffort === 'minimal'
                ? 'low'
                : ['low', 'medium', 'high', 'xhigh'].includes(storedReasoningEffort ?? '')
            ...
            userInstruction: typeof parsed.userInstruction === 'string' &&
                parsed.userInstruction !== LEGACY_DEFAULT_USER_INSTRUCTION
                ? parsed.userInstruction
                : DEFAULT_USER_INSTRUCTION,
            includeUserInstruction: parsed.includeUserInstruction === undefined
                ? true
                : Boolean(parsed.includeUserInstruction),
            enterSubmits: parsed.enterSubmits === undefined ? true : Boolean(parsed.enterSubmits),
...
export function getSelectAboveShortcut() {
