...
export class CodexRunner {
    ...
    async runAgent(prompt, options, onStatus) {
        await this.services.ready;
        if (!this.services.terminals.isAvailable()) {
            throw new Error('The Jupyter terminal service is unavailable on this server. Agent mode requires TerminalManager access.');
        }
        if (this.terminal) {
            try {
...
`;
        ...
        const completion = new Promise((resolve, reject) => {
            const finish = () => {
                ...
                try {
                    ...
                    reject(new Error(`The agent terminal returned an invalid result: ${error instanceof Error ? error.message : String(error)}`));
                }
            };
            ...
            terminal.disposed.connect(() => {
                if (!settled) {
                    settled = true;
                    reject(new Error(`The agent terminal closed before Codex returned a result.\n${stripAnsi(output.slice(-5000))}`));
                }
            });
        });
    ...
    async ensureSession(kernelName) {
    ...
    async validatePythonSources(sources, kernelName) {
        if (!sources.length) {
            return;
        }
...
`;
        let output = '';
        const future = session.kernel.requestExecute({ code: pythonCode, stop_on_error: true }, false);
        ...
        if (start < 0 || end < 0 || end <= start) {
            throw new Error(`Python syntax validation returned no parseable result.\n${cleaned.slice(-4000)}`);
        }
        const payloadText = cleaned
        ...
        const payload = JSON.parse(payloadText);
        if (!payload.ok) {
            throw new Error(`Patch not applied because generated Python is syntactically invalid:\n${(payload.errors ?? ['Unknown syntax error.']).join('\n\n')}`);
        }
    }
    async run(prompt, kernelName, options, onStatus) {
        const session = await this.ensureSession(kernelName);
        if (!session.kernel) {
            throw new Error('The helper session has no kernel.');
        }
