...
function textFromMimeBundle(data) {
...
function outputToText(output) {
    ...
    if (output.output_type === 'execute_result') {
        return `[execute_result]\n${textFromMimeBundle((output.data ?? {}))}`;
    }
    if (output.output_type === 'display_data') {
        const plain = textFromMimeBundle((output.data ?? {}));
        return plain
            ? `[display_data]\n${plain}`
...
function collectOutputText(model, maxChars) {
    const outputBlocks = model.outputs.toJSON()
        .map(outputToText)
        .filter(Boolean);
...
export function countNotebookContextCells(panel, options) {
...
export function collectNotebookContext(panel, options) {
    const notebook = panel.content;
    const activeIndex = notebook.activeCellIndex;
    const selectedIndexes = notebook.widgets
        .map((cell, index) => (notebook.isSelectedOrActive(cell) ? index : -1))
    ...
    const characterCount = cells.reduce((sum, cell) => sum + cell.source.length + cell.outputText.length, 0);
    return {
...
export function editPolicyDescription(options) {
...
export function formatNotebookContext(context) {
    return context.cells
        ...
        const execution = cell.cellType === 'code'
...
export function buildAssistantPrompt(task, context, options, manualMode, userInstruction = '') {
