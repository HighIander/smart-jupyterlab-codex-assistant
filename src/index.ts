...
import {
  ...
  SessionHistoryStore,
  createAssistantMainWidget,
  createHistoryMainWidget
} from './widgets';
...
const plugin: JupyterFrontEndPlugin<void> = {
  ...
  activate: (
    ...
    let assistantWidget: MainAreaWidget<AssistantContent> | null = null;
    let historyWidget: MainAreaWidget<SessionHistoryContent> | null = null;
    const historyStore = new SessionHistoryStore();

    const activateAssistant = (): void => {
      (app.shell as ILabShell).expandRight();
      if (assistantWidget && !assistantWidget.isDisposed) {
        app.shell.activateById(assistantWidget.id);
      }
    };

    const openHistory = (entryId?: number): void => {
      if (!historyWidget || historyWidget.isDisposed) {
        const content = new SessionHistoryContent(
          notebooks,
          historyStore,
          async entryId => {
            const assistant =
              assistantWidget && !assistantWidget.isDisposed
    ...
    const openAssistant = (): AssistantContent => {
      if (!assistantWidget || assistantWidget.isDisposed) {
        const content = new AssistantContent(
          notebooks,
          app.serviceManager,
          activateAssistant,
          historyStore,
    ...
    document.addEventListener(
      ...
      event => {
        ...
        const directive = extractAssistantDirective(
          activeCell.model.sharedModel.getSource()
        );
        if (!directive) {
          return;
        }
