...
import {
  ...
  collectNotebookContext,
  countNotebookContextCells,
  formatNotebookContext,
  sourceHash
} from './context';
...
type StatusLevel = 'normal' | 'success' | 'error';

function button(
  label: string,
  className = 'jna-Button',
  title = ''
): HTMLButtonElement {
...
function decodeSourceTransportEscapes(source: string): string {
  let result = '';
  let state:
    | 'code'
    | 'comment'
...
function repairSourceFields(candidate: string): string {
...
function parsePatchCandidate(candidate: string): NotebookPatch {
  try {
    return JSON.parse(candidate) as NotebookPatch;
  } catch (strictError) {
    const repaired = repairSourceFields(candidate);
    try {
      return JSON.parse(repaired) as NotebookPatch;
    } catch {
      throw strictError;
    }
  }
...
function patchFromText(text: string): NotebookPatch {
  let candidate = text.trim();
  const tagMatch = candidate.match(
    /<jupyter_patch>\s*([\s\S]*?)\s*<\/jupyter_patch>/i
  );
  if (tagMatch) {
...
interface CellDisplayReference {
  label: string;
  metadata: string;
  preview: string;
  title: string;
...
function sourcePreview(source: string, maxLength = 110): string {
...
function cellDisplayReference(
  cellId: string,
  context: NotebookContext,
  panel: NotebookPanel | null
): CellDisplayReference {
  if (!cellId) {
    return {
      label: 'notebook end',
      metadata: '',
  ...
  const source =
    snapshot?.source ?? currentCell?.model.sharedModel.getSource() ?? '';
  const executionCount =
    snapshot?.executionCount ??
    ((currentCell?.model as unknown as { executionCount?: number | null })
      ?.executionCount ?? null);
...
function operationLabel(
  ...
  const reference = cellDisplayReference(
    operation.reference_cell_id,
    context,
    panel
  ).label;
...
function appendCellReference(
  parent: HTMLElement,
  role: string,
  reference: CellDisplayReference
): void {
...
const PROJECT_README_URL =
  'https://github.com/HighIander/jupyter-notebook-assistant#readme';
const PROJECT_LICENSE_URL =
  'https://github.com/HighIander/jupyter-notebook-assistant/blob/main/LICENSE';

const AGENT_MODE_WARNING =
  'Danger starts here. Change only settings you fully understand. ' +
...
function notebookDirectoryPath(panel: NotebookPanel): string {
...
function resolveProjectRoot(panel: NotebookPanel, configuredRoot: string): string {
  const trimmed = configuredRoot.trim() || './';
  if (trimmed.startsWith('/')) {
    return normalizeFilesystemPath(trimmed);
  }
  return normalizeFilesystemPath(
    PathExt.join(notebookDirectoryPath(panel), trimmed)
  );
}

function parentDirectory(path: string, levels: number): string {
  let result = path;
...
function pathRelativeToProjectRoot(
  projectRoot: string,
  targetPath: string
): string {
...
function accessLabel(access: AgentAccess): string {
  if (access === 'write') {
    return 'write';
  }
  if (access === 'read') {
...
type LineDiffKind = 'context' | 'remove' | 'add';
...
function splitDiffLines(source: string): string[] {
 ...
 */
function calculateLineDiff(oldSource: string, newSource: string): LineDiffEntry[] {
...
function lineDiff(oldSource: string, newSource: string): HTMLElement {
...
type NotebookSnapshotCell = Record<string, unknown> & {
  id?: string;
  cell_type?: string;
  source?: string | string[];
};

type NotebookSnapshot = Record<string, unknown> & {
  cells: NotebookSnapshotCell[];
};

function readNotebookSnapshot(value: unknown): NotebookSnapshot {
  if (
    value === null ||
    typeof value !== 'object' ||
  ...
  return value as NotebookSnapshot;
}

function snapshotCellId(cell: NotebookSnapshotCell): string {
  return typeof cell.id === 'string' ? cell.id : '';
}

function snapshotCellSource(cell: NotebookSnapshotCell): string {
...
function setSnapshotCellSource(
  cell: NotebookSnapshotCell,
  source: string
): void {
  cell.source = source;
}

function snapshotCellWithoutSource(cell: NotebookSnapshotCell): string {
...
function snapshotWithoutCells(snapshot: NotebookSnapshot): string {
  const comparable = cloneJson(snapshot) as Record<string, unknown>;
...
function insertSnapshotCellByTargetOrder(
  cells: NotebookSnapshotCell[],
  cell: NotebookSnapshotCell,
  targetOrder: string[]
): void {
  ...
  const existingIndex = cells.findIndex(
    candidate => snapshotCellId(candidate) === id
  );
  if (existingIndex >= 0) {
  ...
  for (let index = targetIndex + 1; index < targetOrder.length; index += 1) {
    const nextIndex = cells.findIndex(
      candidate => snapshotCellId(candidate) === targetOrder[index]
    );
    if (nextIndex >= 0) {
  ...
  for (let index = targetIndex - 1; index >= 0; index -= 1) {
    const previousIndex = cells.findIndex(
      candidate => snapshotCellId(candidate) === targetOrder[index]
    );
    if (previousIndex >= 0) {
...
function makeSnapshotCell(
  operation: PatchOperation,
  id: string
): NotebookSnapshotCell {
 ...
 */
function simulatePatchOnSnapshot(
  ...
  for (const operationIndex of selectedOperationIndexes) {
    ...
    if (
      ...
      const destination =
        operation.operation === 'insert_before' ? sourceIndex : sourceIndex + 1;
    ...
    if (
      operation.operation === 'move_before' ||
      operation.operation === 'move_after'
    ) {
      if (sourceIndex < 0) {
        throw new Error(`The source of operation ${operationIndex + 1} no longer exists.`);
      }
      const referenceIndex = cells.findIndex(
        cell => snapshotCellId(cell) === operation.reference_cell_id
      );
      if (referenceIndex < 0) {
        throw new Error(
          `The destination anchor of operation ${operationIndex + 1} no longer exists.`
        );
      }
      const [movedCell] = cells.splice(sourceIndex, 1);
      const adjustedReferenceIndex = cells.findIndex(
        cell => snapshotCellId(cell) === operation.reference_cell_id
      );
      const destination =
        operation.operation === 'move_before'
...
function replaySnapshotDelta(
  ...
  const sourceById = new Map(
    source.cells.map(cell => [snapshotCellId(cell), cell] as const)
  );
  const targetById = new Map(
    target.cells.map(cell => [snapshotCellId(cell), cell] as const)
  );
  ...
  for (const id of targetIds.filter(candidate => sourceIdSet.has(candidate))) {
    ...
    if (!currentCell) {
      throw new Error(
        `A cell changed by the recorded patch (${id}) no longer exists in the current notebook.`
      );
    }
  ...
  const stableIds = longestCommonSubsequence(
    commonSourceOrder,
    commonTargetOrder
  );
  for (const id of commonTargetOrder.filter(candidate => !stableIds.has(candidate))) {
    ...
    if (!currentCell) {
      throw new Error(
        `A moved cell from the recorded patch (${id}) no longer exists in the current notebook.`
      );
    }
...
function notebookSnapshotFingerprint(snapshot: unknown): string {
  return JSON.stringify(snapshot);
}

function notebookSnapshotDiff(
  beforeValue: unknown,
  afterValue: unknown
): HTMLElement {
  ...
  const beforeById = new Map(
    before.cells.map((cell, index) => [snapshotCellId(cell), { cell, index }] as const)
  );
  const afterById = new Map(
    after.cells.map((cell, index) => [snapshotCellId(cell), { cell, index }] as const)
  );
  let changeCount = 0;
  ...
  for (const id of beforeIds.filter(candidate => !afterIdSet.has(candidate))) {
    ...
    appendChange(
      `Remove current Cell ${beforeEntry.index + 1}`,
      snapshotCellSource(beforeEntry.cell),
      ''
    );
  }
  ...
  for (const id of afterIds.filter(candidate => !beforeIdSet.has(candidate))) {
    ...
    appendChange(
      `Add result Cell ${afterEntry.index + 1} (${afterEntry.cell.cell_type ?? 'cell'})`,
      '',
      snapshotCellSource(afterEntry.cell)
    );
  }
    ...
    if (oldSource !== newSource) {
      appendChange(
        beforeEntry.index === afterEntry.index
          ...
          : `Modify current Cell ${beforeEntry.index + 1} → result Cell ${afterEntry.index + 1}`,
        oldSource,
        newSource
      );
    }
    if (
      snapshotCellWithoutSource(beforeEntry.cell) !==
      snapshotCellWithoutSource(afterEntry.cell)
    ) {
      appendChange(
        `Restore non-source data of Cell ${afterEntry.index + 1}`,
        undefined,
        undefined,
        'Outputs, execution count, attachments, or cell metadata differ.'
      );
    }
  }
  ...
  for (const id of commonAfterOrder.filter(candidate => !stableIds.has(candidate))) {
    ...
    appendChange(
      `Move Cell ${beforeEntry.index + 1} → position ${afterEntry.index + 1}`
    );
  }

  if (snapshotWithoutCells(before) !== snapshotWithoutCells(after)) {
    appendChange(
      'Restore notebook-level metadata',
      undefined,
      undefined,
      'Notebook metadata or format fields differ.'
    );
  }
...
export class SessionHistoryStore {
  ...
  private listeners = new Set<SessionHistoryListener>();
  private nextId = 1;
  private conversationNumber = 1;
  private messageNumber = 0;
  private undoLimit = 20;

  getEntries(): SessionHistoryEntry[] {
    return this.entries.slice();
  }

  nextMessagePosition(): { conversationNumber: number; messageNumber: number } {
  ...
  update(
    id: number,
    changes: Partial<Omit<SessionHistoryEntry, 'id'>>
  ): void {
  ...
  private trimUndoSnapshots(): void {
    const entriesWithUndoData = this.entries.filter(
      entry => entry.beforeSnapshot !== null || entry.afterSnapshot !== null
    );
...
export class AssistantContent extends Widget {
  constructor(
    ...
    private readonly requestActivation?: () => void,
    private readonly historyStore: SessionHistoryStore = new SessionHistoryStore(),
    private readonly requestHistoryOpen?: (entryId?: number) => void
  ) {
    super();
    ...
    this.buildUI();
    this.historyUnsubscribe = this.historyStore.subscribe(() =>
      this.syncCurrentHistoryState()
    );
    this.updateModeUI();
  ...
  private loginInstructionsShown: boolean;
  private contextOptions: ContextOptions;
  private assistantShortcut: string;
  private selectAboveShortcut: string;
  private codexReasoningEffort: CodexReasoningEffort;
  private codexSpeed: CodexSpeed;
  private autoApplyAfterParseSetting: boolean;
  private codexAutoApplySetting: boolean;
  private autoRunOnApplySetting: boolean;
  private userInstruction: string;
  private includeUserInstruction: boolean;
  private enterSubmits: boolean;
  private undoHistoryLimit: number;
  private readonly codexRunner: CodexRunner;
  private statusFrame = 0;
  private statusDirty = false;
  private currentPatch: NotebookPatch | null = null;
  private patchContext: NotebookContext | null = null;
  private patchOptions: ContextOptions | null = null;
  private pendingManualContext: NotebookContext | null = null;
  private pendingManualOptions: ContextOptions | null = null;
  private operationChecks: HTMLInputElement[] = [];
  private undoSnapshot: unknown | null = null;
  private busy = false;
  private detailsCollapsed = false;
  private patchApplied = false;
  private currentHistoryEntryId: number | null = null;
  private historyUnsubscribe: (() => void) | null = null;
  private feedbackTimers = new Map<HTMLButtonElement, number>();
  private successBannerTimer: number | null = null;
  private generateLockedUntilInput = false;

  private contentNode!: HTMLDivElement;
  private agentBanner!: HTMLDivElement;
  private successBanner!: HTMLDivElement;
  private modeCodexButton!: HTMLButtonElement;
  private modeChatGPTButton!: HTMLButtonElement;
  private newConversationButton!: HTMLButtonElement;
  private loginButton!: HTMLButtonElement;
  private settingsDetails!: HTMLDetailsElement;
  private taskInput!: HTMLTextAreaElement;
  private codexOptions!: HTMLDivElement;
  private reasoningSelect!: HTMLSelectElement;
  private speedSelect!: HTMLSelectElement;
  private contextButton!: HTMLButtonElement;
  private agentModeButton!: HTMLButtonElement;
  private agentModeToggle!: HTMLButtonElement;
  private codexActions!: HTMLDivElement;
  private chatGPTActions!: HTMLDivElement;
  private generateButton!: HTMLButtonElement;
  private cancelButton!: HTMLButtonElement;
  private workingLabel!: HTMLDivElement;
  private openChatGPTButton!: HTMLButtonElement;
  private copyPromptButton!: HTMLButtonElement;
  private responseInput!: HTMLTextAreaElement;
  private parseButton!: HTMLButtonElement;
  private autoParseAfterPaste!: HTMLInputElement;
  private autoApplyAfterParse!: HTMLInputElement;
  private codexAutoApply!: HTMLInputElement;
  private manualAutoApplyLine!: HTMLLabelElement;
  private codexAutoApplyLine!: HTMLLabelElement;
  private autoRunOnApply!: HTMLInputElement;
  private resultSection!: HTMLDivElement;
  private resultNode!: HTMLDivElement;
  private patchDetailsNode: HTMLDivElement | null = null;
  private detailsToggleButton!: HTMLButtonElement;
  private statusMessage!: HTMLDivElement;
  private targetStatus!: HTMLSpanElement;
  private processStatus!: HTMLSpanElement;
  private historyButton!: HTMLButtonElement;
  private applyButton!: HTMLButtonElement;
  private rejectButton!: HTMLButtonElement;
  private undoButton!: HTMLButtonElement;
  private patchActionRow!: HTMLDivElement;
  ...
  private get panel(): NotebookPanel | null {
  ...
  private buildUI(): void {
    ...
    enterSubmitLine.append(
      enterSubmitRadio,
      document.createTextNode('Enter submits; Shift+Enter inserts a new line')
    );
    ...
    shiftEnterSubmitLine.append(
      shiftEnterSubmitRadio,
      document.createTextNode('Shift+Enter submits; Enter inserts a new line')
    );
    ...
    const userInstructionsButton = button(
      'User instructions…',
      'jna-SettingsMenuAction'
    );
    ...
    this.taskInput.addEventListener('keydown', event => {
      const noSelection =
        this.taskInput.selectionStart === this.taskInput.selectionEnd;
    ...
    const reasoningOptions: Array<[CodexReasoningEffort, string]> = [
      ['low', 'Low'],
      ['medium', 'Medium'],
      ['high', 'High'],
      ['xhigh', 'Extra high']
    ...
    this.speedSelect.onchange = () => {
      this.codexSpeed = this.speedSelect.value as CodexSpeed;
      this.persistSettings();
    };
    ...
    this.codexAutoApplyLine.append(
      this.codexAutoApply,
      document.createTextNode('Automatically apply after patch generation')
    );
    this.codexActions.append(
      codexButtonRow,
      this.workingLabel,
      this.codexAutoApplyLine
    );
    ...
    autoParseLine.append(
      this.autoParseAfterPaste,
      document.createTextNode('Automatically parse after paste')
    );

    this.parseButton = button('Parse pasted response');
    this.parseButton.onclick = () => void this.parseManualResponse();

    this.chatGPTActions.append(
      manualButtons,
      this.responseInput,
      autoParseLine,
      this.parseButton
    ...
    this.detailsToggleButton.onclick = () =>
      this.setDetailsCollapsed(!this.detailsCollapsed);
    ...
    this.manualAutoApplyLine.append(
      this.autoApplyAfterParse,
      document.createTextNode('Automatically apply after parse')
    );
    ...
    runLine.append(
      this.autoRunOnApply,
      document.createTextNode('Automatically run selected cell(s) on apply')
    );
  ...
  private isReadyButton(element: HTMLButtonElement): boolean {
    return !element.hidden && !element.disabled && element.dataset.state === 'ready';
  }

  private shouldSubmitFromInput(event: KeyboardEvent): boolean {
    if (
      event.key !== 'Enter' ||
      event.isComposing ||
      event.ctrlKey ||
  ...
  private runTaskFieldPrimaryAction(): void {
    if (this.mode === 'codex') {
    ...
    const nextButton = [this.openChatGPTButton, this.copyPromptButton].find(
      element => this.isReadyButton(element)
    );
    if (nextButton) {
      nextButton.click();
    }
  ...
  private runPasteFieldPrimaryAction(): void {
  ...
  private setButtonState(
    ...
    state: 'neutral' | 'ready' | 'success' | 'danger',
    label?: string
  ): void {
  ...
  private showPatchAppliedBanner(): void {
  ...
  private setDetailsCollapsed(collapsed: boolean): void {
    ...
    this.detailsToggleButton.setAttribute(
      'aria-label',
      collapsed ? 'Restore diff' : 'Minimize diff'
    );
  }

  private resetManualVisualState(focus: boolean): void {
  ...
  private updateManualTaskState(): void {
    if (this.mode !== 'chatgpt') {
      return;
    }
    ...
    this.setButtonState(
      this.openChatGPTButton,
      hasText ? 'ready' : 'neutral',
      'Open ChatGPT'
    );
    this.setButtonState(this.copyPromptButton, 'neutral', 'Copy prompt');
  ...
  private updateManualPasteState(): void {
    if (this.mode !== 'chatgpt') {
      return;
    }
  ...
  private async openCodexLoginDialog(): Promise<void> {
    ...
    copyButton.onclick = async () => {
      try {
        ...
        this.setStatus(
          `Could not copy the login commands: ${
  ...
  private initializeCodexTaskField(): void {
  ...
  private updateWorkingModeUI(): void {
  ...
  private async confirmAgentActivation(): Promise<boolean> {
    const body = new Widget();
  ...
  private async setWorkingMode(
    ...
    this.setStatus(
      mode === 'agent'
        ...
        : 'Local notebook mode active.',
      '● Ready',
      mode === 'agent' ? 'error' : 'success'
    );
    return true;
  }

  private async toggleAgentMode(): Promise<void> {
    await this.setWorkingMode(
      this.workingMode === 'agent' ? 'local' : 'agent'
    );
  }
  ...
  private createAgentAccessSelect(
    ...
    select.value = value;
    select.onchange = () => onChange(select.value as AgentAccess);
    return select;
  }

  private async openAgentModeDialog(): Promise<void> {
    const draft = cloneAgentPermissions(this.agentPermissions);
    let draftMode: WorkingMode = this.workingMode;
    const body = new Widget();
    ...
    const addAccessRow = (
      ...
      key:
        | 'notebookDirectory'
        | 'notebookSubdirectories'
        | 'referencedInNotebook'
        | 'referencedInReferences'
        | 'parentAccess',
      extra?: HTMLElement
    ): void => {
      const row = document.createElement('div');
      ...
      controls.appendChild(
        this.createAgentAccessSelect(draft[key], value => {
          draft[key] = value;
        })
      );
    ...
    addAccessRow(
      'Project files referenced by those references',
      'referencedInReferences'
    );
    ...
    addAccessRow(
      'All files in parent directories up to the selected number of levels',
      'parentAccess',
      parentLevels
    );
    ...
    const addPermissionCheckbox = (
      labelText: string,
      key: 'addFiles' | 'deleteFiles' | 'runTests' | 'runArbitraryCommands'
    ): void => {
    ...
    const updateCompatibilityWarning = (): void => {
    ...
    policyBoundaryLabel.append(
      policyBoundaryCheckbox,
      document.createTextNode('Enable agent policy boundaries:')
    );
    ...
    const updatePatternToggle = (): void => {
    ...
    const setPatternMode = (mode: AgentPatternMode): void => {
    ...
    patternToggle.onclick = () =>
      setPatternMode(
        draft.patternMode === 'whitelist' ? 'blacklist' : 'whitelist'
      );
    ...
    const updatePolicyBoundaryControls = (): void => {
    ...
    const updateEnforcementNote = (): void => {
    ...
    draft.parentLevels = Math.max(
      0,
      Math.min(20, Math.trunc(Number(draft.parentLevels) || 0))
    );
  ...
  private onNotebookTargetChanged(): void {
    this.statusDirty = true;
    this.scheduleNotebookStatusRefresh();
  }

  private scheduleNotebookStatusRefresh(): void {
  ...
  private refreshNotebookStatus(): void {
  ...
  private requirePanel(): NotebookPanel {
    const panel = this.panel;
    if (!panel) {
  ...
  public async consumeAssistantDirective(): Promise<void> {
    if (this.busy) {
      this.setStatus(
        'The assistant is already processing a request.',
        '● Busy',
        'error'
      );
      return;
    ...
    try {
      ...
      if (!directive) {
        throw new Error(
          'No non-empty @assistant message was found in a comment of the active cell.'
        );
      }
      ...
      this.updateManualTaskState();
      this.setStatus(
        'Moved the @assistant comment into the task field.',
        this.mode === 'codex' ? '● Generating patch' : '● Copying prompt',
        'success'
      );
  ...
  private buildPrompt(manualMode: boolean): {
    context: NotebookContext;
    options: ContextOptions;
    prompt: string;
  } {
    ...
    return {
      ...
      prompt: buildAssistantPrompt(
        ...
        options,
        manualMode,
        this.includeUserInstruction ? this.userInstruction : ''
      )
    };
  }
  ...
  private buildAgentPrompt(): {
    ...
    const permissionLines = [
      `Notebook directory (${notebookDirectory}): ${accessLabel(
        permissions.notebookDirectory
      )}`,
      `Notebook subdirectories: ${accessLabel(
        permissions.notebookSubdirectories
      )}`,
      `Project files referenced in the current notebook: ${accessLabel(
        permissions.referencedInNotebook
      )}`,
      `Project files referenced recursively by those references: ${accessLabel(
        permissions.referencedInReferences
      )}`,
      `All files in parent directories up to ${permissions.parentLevels} level(s) above project root (${parentRoot}): ${accessLabel(
      ...
      `Run arbitrary terminal commands: ${
        permissions.runArbitraryCommands ? 'allowed' : 'forbidden'
      }`,
      `Codex OS sandbox: ${
      ...
      `Agent policy boundaries: ${
        permissions.policyBoundariesEnabled ? 'enabled' : 'disabled'
      }`,
    ...
    const sandbox = permissions.disableCodexSandbox
      ...
      : writableAccess || permissions.addFiles || permissions.deleteFiles
        ? 'workspace-write'
        : 'read-only';
    const additionalWriteDirectories = new Set<string>();
    if (
      permissions.notebookDirectory === 'write' ||
      permissions.notebookSubdirectories === 'write'
    ) {
      if (notebookDirectory !== projectRoot) {
        additionalWriteDirectories.add(
          pathRelativeToProjectRoot(projectRoot, notebookDirectory)
        );
      }
    }
    if (permissions.parentAccess === 'write' && permissions.parentLevels > 0) {
      if (parentRoot !== projectRoot) {
        additionalWriteDirectories.add(
          pathRelativeToProjectRoot(projectRoot, parentRoot)
        );
      }
    }
  ...
  private async runAgentTask(): Promise<void> {
    ...
    try {
      ...
      this.clearResultState();
      this.setStatus(
        'Preparing project-agent context and permissions…',
        '● Collecting context'
      );
      const result = await this.codexRunner.runAgent(
        built.prompt,
        {
      ...
      this.updateGenerateButtonState();
      this.setStatus(
        'Agent run completed. Review the reported file and command changes carefully.',
        '● Agent completed',
        'success'
      );
    } catch (error) {
  ...
  private prepareForManualResponse(): void {
  ...
  private copyTextSynchronously(text: string): boolean {
  ...
  private async writeClipboard(text: string): Promise<void> {
    ...
    if (!this.copyTextSynchronously(text)) {
      throw new Error(
        'The browser did not permit clipboard access. Allow clipboard access for this JupyterHub and try again.'
      );
    }
  }
  ...
  private async copyPrompt(): Promise<void> {
    try {
      ...
      this.setStatus(
        'Prompt copied. Paste it into ChatGPT, then paste the response below.',
        '● Prompt copied',
        'success'
      );
    } catch (error) {
  ...
  private async openChatGPT(): Promise<void> {
    let prompt = '';
    try {
    ...
    const chatWindow = window.open(
      'https://chatgpt.com/',
      'jupyter-notebook-assistant-chatgpt'
    );
    ...
    if (!chatWindow) {
      ...
      this.setStatus(
        copied
          ...
          : 'The browser blocked both the ChatGPT tab and clipboard access. Allow pop-ups and clipboard access for this JupyterHub.',
        '● Error',
        'error'
      );
      return;
    ...
    this.setStatus(
      copied
        ...
        : 'ChatGPT opened, but clipboard access was denied. Press Copy prompt after allowing clipboard access.',
      copied ? '● ChatGPT opened' : '● Clipboard blocked',
      copied ? 'success' : 'error'
    );
  }

  private async generateCodexPatch(): Promise<void> {
    ...
    try {
      ...
      const kernelName =
        panel.sessionContext.session?.kernel?.name || 'python3';
      this.setBusy(true);
  ...
  private async cancelCodex(): Promise<void> {
    await this.codexRunner.cancel();
    this.setStatus('Cancellation requested.', '● Cancelling');
  }

  private async parseManualResponse(): Promise<void> {
    if (this.mode !== 'chatgpt') {
      return;
    }
    try {
      ...
      this.setStatus(
        'Pasted ChatGPT response parsed. Review the operations before applying them.',
        '● Patch available',
        'success'
      );
      window.setTimeout(() => {
  ...
  private renderEmptyResult(): void {
    this.resultNode.replaceChildren();
    this.patchDetailsNode = null;
    this.detailsCollapsed = false;
    const empty = document.createElement('div');
  ...
  private setApplyButtonReady(ready: boolean): void {
  ...
  private resetAppliedButtonForEditing(): void {
  ...
  private syncCurrentHistoryState(): void {
  ...
  private renderPatch(
    ...
    for (const [index, operation] of patch.operations.entries()) {
      ...
      label.append(
        checkbox,
        document.createTextNode(` ${operationLabel(operation, context, panel)}`)
      );
      header.appendChild(label);
      wrapper.appendChild(header);

      const targetReference = cellDisplayReference(
        operation.cell_id,
        context,
        panel
      );
      const destinationReference = cellDisplayReference(
        operation.reference_cell_id,
        context,
        panel
      );
      ...
      if (panel && operation.cell_id) {
        const oldCell = panel.content.widgets.find(
          cell => cell.model.id === operation.cell_id
        );
  ...
  private currentCellIndex(panel: NotebookPanel, cellId: string): number {
    return panel.content.model?.sharedModel.cells.findIndex(
      cell => cell.id === cellId
    ) ?? -1;
  }
  ...
  private validateOperation(
    ...
    if (options.editScope === 'active') {
      if (structural || operation.cell_id !== activeId) {
        throw new Error(
          `Operation “${operationLabel(operation, context, panel)}” exceeds the active-cell-only edit scope.`
        );
      }
    } else if (options.editScope === 'selected') {
      if (structural || !selectedIds.has(operation.cell_id)) {
        throw new Error(
          `Operation “${operationLabel(operation, context, panel)}” exceeds the selected-cells edit scope.`
        );
      }
    }

    if (
      operation.cell_type === 'markdown' &&
      !options.allowMarkdownChanges
    ) {
      throw new Error('The patch changes Markdown, but Markdown changes are disabled.');
    }
    if (
      ['delete', 'move_before', 'move_after'].includes(operation.operation) &&
    ...
    if (operation.operation !== 'append') {
      ...
      if (original) {
        const currentSource =
          panel.content.widgets[index]?.model.sharedModel.getSource() ?? '';
        if (sourceHash(currentSource) !== original.sourceHash) {
          throw new Error(
            `Cell ${original.index + 1} changed after the patch request. Generate the patch again.`
          );
        }
      }
    }
    ...
    if (['move_before', 'move_after'].includes(operation.operation)) {
      if (this.currentCellIndex(panel, operation.reference_cell_id) < 0) {
        const destination = cellDisplayReference(
          operation.reference_cell_id,
          context,
          panel
        );
        throw new Error(
          `Destination anchor ${destination.label} no longer exists.`
        );
      }
    }
  }

  private makeCellData(operation: PatchOperation): Record<string, unknown> {
    if (operation.cell_type === 'markdown') {
      return {
        cell_type: 'markdown',
        source: operation.source,
  ...
  private async applyOperationsToPanel(
    panel: NotebookPanel,
    patch: NotebookPatch,
    context: NotebookContext,
    options: ContextOptions,
    ...
    selectedOperations: PatchOperation[];
  }> {
    ...
    selectedOperations.forEach(operation =>
      this.validateOperation(panel, operation, context, options)
    );
    ...
    const pythonSources = selectedOperations
      .filter(
        operation =>
          operation.cell_type === 'code' &&
          ['replace', 'insert_before', 'insert_after', 'append'].includes(
            operation.operation
          )
      )
      .map(operation => ({
    ...
    if (pythonSources.length) {
      this.setStatus(
        `Validating ${pythonSources.length} modified Python code cell(s)…`,
        '● Validating syntax'
      );
    ...
    shared.transact(() => {
      for (const operation of selectedOperations) {
        ...
        if (operation.operation === 'replace') {
          ...
          const inserted = shared.insertCell(
            index,
            this.makeCellData(operation) as never
          );
          ...
          const inserted = shared.addCell(this.makeCellData(operation) as never);
          changedCellIds.add(inserted.id);
          ...
          const referenceIndex = shared.cells.findIndex(
            cell => cell.id === operation.reference_cell_id
          );
          let destination =
            operation.operation === 'move_before'
              ? referenceIndex
  ...
  private async runChangedCodeCells(
    panel: NotebookPanel,
    changedCellIds: Set<string>
  ): Promise<boolean | null> {
    const cellsToRun = panel.content.widgets.filter(
      cell => changedCellIds.has(cell.model.id) && cell.model.type === 'code'
    );
    ...
    this.setStatus(
      `Executing ${cellsToRun.length} modified code cell(s)…`,
      '● Executing'
    );
   ...
   */
  async applyHistoryEntry(entryId: number): Promise<string> {
    const entry = this.historyStore
      .getEntries()
    ...
    this.tracker.forEach(candidate => {
      if (
        candidate.context.path === entry.context.notebookName ||
    ...
    if (!panel) {
      throw new Error(
        `Notebook “${entry.context.notebookName}” is not open. Open it before applying this patch.`
      );
    }

    const selectedOperationIndexes = entry.selectedOperationIndexes.length
      ? entry.selectedOperationIndexes.filter(
          index => index >= 0 && index < entry.patch.operations.length
        )
      : entry.patch.operations.map((_operation, index) => index);

    const result = await this.applyOperationsToPanel(
      panel,
      entry.patch,
      entry.context,
      entry.options,
    ...
    let message =
      `Applied ${result.selectedOperations.length} operation(s) from patch ` +
  ...
  private async applyPatch(): Promise<void> {
    ...
    try {
      if (this.patchApplied) {
        throw new Error(
          'This patch has already been applied. Undo it or parse/generate a new patch before applying again.'
        );
      }
      const panel = this.requirePanel();
      ...
      this.setStatus(
        `Applied ${result.selectedOperations.length} notebook operation(s).`,
        '● Applied',
        'success'
      );
      this.showPatchAppliedBanner();
      ...
      if (this.autoRunOnApply.checked) {
        const success = await this.runChangedCodeCells(
          panel,
        ...
        if (success !== null) {
          this.setStatus(
            success
              ...
              : 'Changes applied; at least one modified cell did not complete successfully.',
            success ? '● Ready' : '● Execution error',
  ...
  private undoChanges(): void {
    if (this.currentHistoryEntryId === null) {
      this.setStatus(
        'No applied AI patch is available in the current conversation.',
        '● Ready',
        'error'
      );
      return;
    ...
    if (!this.requestHistoryOpen) {
      this.setStatus(
        'Session AI history is unavailable.',
        '● Error',
        'error'
      );
      return;
    ...
    this.setStatus(
      'Choose “Restore previous state” or “Replay inverse patch” in Session AI history. Both actions show a diff preview before changing the notebook.',
      '● Ready',
      'success'
    );
  }

  private async openUserInstructionsDialog(): Promise<void> {
    ...
    enabledLabel.append(
      enabledInput,
      document.createTextNode('Include these instructions in every generated prompt')
    );
    ...
    this.setStatus(
      this.includeUserInstruction
        ...
        : 'User instructions saved but disabled.',
      '● Ready',
      'success'
    );
  }

  private async openContextDialog(): Promise<void> {
    ...
    const updatePermissionSummary = (): void => {
    ...
    const addRadio = (
      labelText: string,
      value: ContextOptions['editScope']
    ): void => {
      const label = document.createElement('label');
    ...
    const addAllowedCheckbox = (
      labelText: string,
      key: 'allowMarkdownChanges' | 'allowDeleteMove'
    ): void => {
      const label = document.createElement('label');
      label.className = 'jna-OptionLine';
      const input = document.createElement('input');
    ...
    const updateSummary = (): NotebookContext => {
      ...
      summary.textContent = `${context.cells.length} cells · ${formatSize(
        totalCharacters
      )} · estimated ${Math.ceil(totalCharacters / 4).toLocaleString()} tokens` +
        (this.includeUserInstruction
    ...
    if (result.button.accept) {
      try {
        ...
        if (normalizedSelectAboveShortcut === 'Ctrl+Shift+A') {
          throw new Error(
            'Ctrl+Shift+A is reserved by the browser and cannot reach JupyterLab. Use Ctrl+Alt+A or another shortcut.'
          );
        }
      ...
      this.setStatus(
        `Notebook context and permissions saved (${shortcutMessages.join('; ')}).`,
        '● Ready',
        'success'
      );
    }
...
export class SessionHistoryContent extends Widget {
  constructor(
    ...
    private readonly requestApply?: (entryId: number) => Promise<string>,
  ...
  private metadataNode!: HTMLDivElement;
  private positionNode!: HTMLDivElement;
  private previousButton!: HTMLButtonElement;
  private nextButton!: HTMLButtonElement;
  private slider!: HTMLInputElement;
  private contentNode!: HTMLDivElement;
  private applyNowButton!: HTMLButtonElement;
  ...
  private buildUI(): void {
    ...
    this.applyNowButton = button(
      'Apply now',
      'jna-Button',
      'Preview and apply this previously generated patch'
    );
    this.restoreButton = button(
      'Restore previous state',
      'jna-Button',
      'Preview and restore an exact recorded notebook state'
    );
    this.replayButton = button(
      'Replay inverse patch',
      'jna-Button',
      'Preview and replay only the recorded patch delta'
    );
  ...
  private onHistoryChanged(selectNewest = false): void {
  ...
  private move(offset: number): void {
    ...
    this.currentIndex = Math.max(
      0,
      Math.min(entries.length - 1, this.currentIndex + offset)
    );
    this.slider.value = String(this.currentIndex);
    this.renderCurrent();
  }

  private currentEntry(): SessionHistoryEntry | null {
  ...
  private renderCurrent(): void {
    ...
    for (const [index, operation] of entry.patch.operations.entries()) {
      ...
      const oldSource =
        entry.context.cells.find(cell => cell.id === operation.cell_id)?.source ?? '';
      if (operation.operation === 'replace') {
        wrapper.appendChild(lineDiff(oldSource, operation.source));
      } else if (
        operation.operation === 'insert_before' ||
  ...
  private findNotebook(entry: SessionHistoryEntry): NotebookPanel | null {
    let match: NotebookPanel | null = null;
    this.tracker.forEach(panel => {
      if (
        panel.context.path === entry.context.notebookName ||
  ...
  private requireNotebook(entry: SessionHistoryEntry): NotebookPanel {
    const panel = this.findNotebook(entry);
    if (!panel?.content.model?.sharedModel) {
      throw new Error(
        `Notebook “${entry.context.notebookName}” is not open. Open it before using this history action.`
      );
    }
    return panel;
  }

  private snapshotPanel(panel: NotebookPanel): NotebookSnapshot {
    const shared = panel.content.model?.sharedModel;
    if (!shared) {
  ...
  private ensureNotebookUnchanged(
    ...
    const currentFingerprint = notebookSnapshotFingerprint(
      this.snapshotPanel(panel)
    );
    if (currentFingerprint !== expectedFingerprint) {
      throw new Error(
        'The notebook changed while the preview was open. Open the preview again before confirming.'
      );
    }
  }
  ...
  private async confirmChange(
    ...
    targetSnapshot: NotebookSnapshot,
    confirmLabel: string,
    destructive: boolean,
    warning: string
  ): Promise<boolean> {
  ...
  private setActionsBusy(busy: boolean): void {
  ...
  private async applyCurrent(): Promise<void> {
    ...
    try {
      ...
      const selectedOperationIndexes = entry.selectedOperationIndexes.length
        ...
        : entry.patch.operations.map((_operation, index) => index);
      const targetSnapshot = simulatePatchOnSnapshot(
        currentSnapshot,
        entry.patch,
        selectedOperationIndexes,
        `jna-history-preview-${entry.id}`
      );
      const fingerprint = notebookSnapshotFingerprint(currentSnapshot);
      const confirmed = await this.confirmChange(
        `Apply patch ${entry.conversationNumber}|${entry.messageNumber}?`,
        'This patch has not been applied yet. The preview is calculated against the notebook as it is now.',
        currentSnapshot,
        targetSnapshot,
        'Apply now',
        false,
        'Cell permissions, original source hashes, and Python syntax are checked again after confirmation.'
      );
      if (!confirmed) {
    ...
    try {
      ...
      const confirmed = await this.confirmChange(
        restoreBefore
          ...
          : `Restore state after patch ${entry.conversationNumber}|${entry.messageNumber}?`,
        restoreBefore
          ? 'The complete notebook will be returned to the exact state recorded immediately before this patch was applied.'
          : 'The complete notebook will be returned to the exact state recorded immediately after this patch was applied.',
        currentSnapshot,
        targetSnapshot,
        restoreBefore ? 'Restore previous state' : 'Restore applied state',
        true,
        'This is a full-state restoration. All notebook edits made after the recorded state—including unrelated edits—will be replaced.'
      );
      if (!confirmed) {
      ...
      this.ensureNotebookUnchanged(panel, fingerprint);
      shared.fromJSON(cloneJson(targetSnapshot) as never);
      this.historyStore.update(entry.id, {
      ...
      this.setStatus(
        restoreBefore
          ...
          : `Restored the complete state after patch ${entry.conversationNumber}|${entry.messageNumber}.`,
        false
      );
    } catch (error) {
      this.setStatus(error instanceof Error ? error.message : String(error), true);
  ...
  private async replayCurrent(): Promise<void> {
    ...
    try {
      if (entry.beforeSnapshot === null || entry.afterSnapshot === null) {
        throw new Error(
          'The before/after snapshots needed to replay this patch are no longer stored.'
        );
      }
      ...
      const currentSnapshot = this.snapshotPanel(panel);
      const replayedSnapshot = replaySnapshotDelta(
        currentSnapshot,
        sourceSnapshot,
        targetSnapshot
      );
      ...
      const confirmed = await this.confirmChange(
        inverse
          ...
          : `Replay patch ${entry.conversationNumber}|${entry.messageNumber}?`,
        inverse
          ? 'Only the recorded patch operations will be reversed on top of the current notebook.'
          : 'Only the recorded patch operations will be replayed on top of the current notebook.',
        currentSnapshot,
        replayedSnapshot,
        inverse ? 'Replay inverse patch' : 'Replay patch',
        true,
        'Unrelated current edits are retained. Sources and structural changes touched by this patch are overwritten or replayed and may leave the notebook inconsistent; review the diff carefully.'
      );
      if (!confirmed) {
      ...
      this.ensureNotebookUnchanged(panel, fingerprint);
      shared.fromJSON(cloneJson(replayedSnapshot) as never);
      this.historyStore.update(entry.id, {
      ...
      this.setStatus(
        inverse
          ...
          : `Replayed patch ${entry.conversationNumber}|${entry.messageNumber} while retaining unrelated current edits.`,
        false
      );
    } catch (error) {
      this.setStatus(error instanceof Error ? error.message : String(error), true);
...
export function createHistoryMainWidget(
  content: SessionHistoryContent
): MainAreaWidget<SessionHistoryContent> {
...
export function createAssistantMainWidget(content: AssistantContent): MainAreaWidget<AssistantContent> {
  const widget = new MainAreaWidget({ content });
